...
export default async function Dashboard() {
  ...
  const tasks = await db.task.findMany({ where: { userId: user.id }, orderBy: [{ dueDate: { sort: "asc", nulls: "last" } }, { createdAt: "desc" }] });
  const open = tasks.filter(t => t.status !== "DONE");
  const done = tasks.length - open.length;
  const due = open.filter(t => t.dueDate?.toISOString().slice(0, 10) === today).length;
  const overdue = open.filter(t => t.dueDate && t.dueDate.toISOString().slice(0, 10) < today).length;
  ...
  return <>
    ...
    <header className="page-heading"><div><span className="eyebrow">היום שלך במבט אחד</span><h1>שלום, {user.name.split(" ")[0]}<span className="hello-dot">.</span></h1><p>כל מה שחשוב לך, במקום אחד וברור.</p></div></header>
    ...
    <section className="module-grid" aria-label="התחומים שלך">
      <Link href="/tasks" className="module-card tasks-card"><div className="card-top"><span className="module-icon"><CheckCheck/></span><span className="card-label">משימות<small>מתקדמים צעד אחר צעד</small></span><ArrowUpRight className="card-arrow" size={20}/></div><div className="task-card-body"><div><div className="big-number">{open.length}<span>משימות פתוחות</span></div><p>{due ? `${due} משימות מתוכננות להיום` : "אפשר לתכנן את הצעד הבא"}</p></div><div className="progress-ring" style={{ "--progress": `${percent}%` } as React.CSSProperties}><div><strong>{percent}%</strong><span>הושלמו</span></div></div></div><div className="card-metrics"><span><b>{due}</b>להיום</span><span><b>{overdue}</b>באיחור</span><span><b>{urgent}</b>דחופות</span></div><div className="card-footer">פתיחת משימות<ArrowRight size={16}/></div></Link>
