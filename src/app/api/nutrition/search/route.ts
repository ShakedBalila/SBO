...
export async function GET(request:Request){
  ...
  if(query.length<2||query.length>100)return NextResponse.json({foods:[]});
  ...
  const cached=local.map(food=>({...food,source:"מאגר SBO · אומדן למזון כללי"}));
  if(cached.length&&new URL(request.url).searchParams.get("online")!=="1")return NextResponse.json({foods:cached,hasOnline:true});
  const translations:Record<string,string>={"שווארמה":"shawarma","שוארמה":"shawarma","חזה עוף":"chicken breast"};
  try{
    const url=new URL("https://world.openfoodfacts.org/cgi/search.pl");
    url.searchParams.set("search_terms",translations[normalized]??query);url.searchParams.set("search_simple","1");url.searchParams.set("action","process");url.searchParams.set("json","1");url.searchParams.set("page_size","8");url.searchParams.set("fields","code,product_name,product_name_he,brands,nutriments");
    const response=await fetch(url,{headers:{"User-Agent":"SBO/1.0 (sbo-pi.vercel.app)"},signal:AbortSignal.timeout(8000),next:{revalidate:3600}});if(!response.ok)throw new Error("search failed");
    const body=await response.json() as {products?:Array<Record<string,unknown>>};
    const foods=(body.products??[]).filter(product=>{const n=product.nutriments as Record<string,unknown>|undefined;return n&&["energy-kcal_100g","proteins_100g","carbohydrates_100g","fat_100g"].every(key=>n[key]!==null&&n[key]!==undefined&&Number.isFinite(Number(n[key]))&&Number(n[key])>=0);}).map(product=>{const nutrients=(product.nutriments??{}) as Record<string,unknown>;return {id:String(product.code??crypto.randomUUID()),source:"Open Food Facts",sourceUrl:`https://world.openfoodfacts.org/product/${encodeURIComponent(String(product.code??""))}`,name:String(product.product_name_he||product.product_name||"מוצר ללא שם"),brand:String(product.brands??""),calories:value(nutrients["energy-kcal_100g"]),proteinG:value(nutrients.proteins_100g),carbsG:value(nutrients.carbohydrates_100g),fatG:value(nutrients.fat_100g)};}).filter(food=>food.calories||food.proteinG||food.carbsG||food.fatG);
    return NextResponse.json({foods:[...cached,...foods]});
  }catch{return NextResponse.json({foods:cached,error:cached.length?"המאגר המקוון אינו זמין כרגע; מוצגות תוצאות מהמאגר המובנה.":"המאגר המקוון אינו זמין כרגע. נסה שוב בעוד רגע."});}
}
