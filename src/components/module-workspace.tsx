...
function RecordPanel({ kind, title, action, fields, records, defaults, describe, disabled = false, externalDraft, onDraftConsumed, confirmDeletion = true, hideAction = false, openSignal = 0 }: {
  ...
  return <section className={`journal-panel ${kind==='water'?'water-journal':''} ${kind==='vehicles'?'vehicles-panel':''}`}>
    ...
    {kind!=='vehicles'&&<p className="feedback" role="status">{message}</p>}
...
export function ModuleWorkspace({ module, summary, water, vehicles, fuel, nutrition, policies, reminders, expenses, fuelPrice }: { module: 'water' | 'car' | 'nutrition'; summary: Summary; water: ModuleRecord[]; vehicles: ModuleRecord[]; fuel: ModuleRecord[]; nutrition: ModuleRecord[]; policies: ModuleRecord[]; reminders: ModuleRecord[]; expenses:ModuleRecord[]; fuelPrice: FuelPrice | null }) {
  ...
  return <div className={`module-workspace ${module}-workspace`}>
    ...
    <section className="module-stats" aria-label={`סיכום ${title}`}>
      ...
      {module === 'car' && <><FuelPriceCard data={fuelPrice} format={formatMoney}/><div><small>עלות הדלק החודש</small><strong>{formatMoney(summary.fuelCost)}</strong></div><div className="car-vehicle-stat"><small>פרטי הרכב</small><strong className="vehicle-summary">{vehicles[0]?.name||'טרם הוגדר רכב'}</strong><small>{vehicles[0]?[vehicles[0].year&&vehicles[0].roadMonth?`${String(vehicles[0].roadMonth).padStart(2,'0')}/${vehicles[0].year}`:null,vehicles[0].licensePlate].filter(Boolean).join(' · '):'הוספת רכב תציג כאן את פרטיו'}</small></div><CarNotifications summary={summary}/></>}
    ...
    {module === 'car' && <>
      <RecordPanel kind="vehicles" title="רכבים" action="הוספת רכב" records={vehicles} fields={[{key:'name',label:'סוג הרכב',max:100},{...numberField('year','שנת עלייה לכביש',2100,1900),compact:true},{key:'roadMonth',label:'חודש עלייה לכביש',type:'number',options:Array.from({length:12},(_,index)=>({value:String(index+1),label:String(index+1).padStart(2,'0')})),compact:true},{key:'licensePlate',label:'מספר לוחית רישוי רכב',type:'license'},{...numberField('odometerKm','קילומטראז׳ נוכחי',10000000),compact:true},{...numberField('fuelTankLiters','נפח מכל הדלק (ליטר)',1000,1,'0.1'),compact:true}]} defaults={{name:'',licensePlate:'',year:new Date().getFullYear(),roadMonth:new Date().getMonth()+1,odometerKm:0,fuelTankLiters:''}} describe={row=>({title:String(row.name),detail:[row.year&&row.roadMonth?`עלה לכביש ${String(row.roadMonth).padStart(2,'0')}/${row.year}`:row.year,row.licensePlate,`${Number(row.odometerKm).toLocaleString('he-IL')} ק״מ`,row.fuelTankLiters?`מכל ${row.fuelTankLiters} ליטר`:null,...(['Mandatory','Third party','Comprehensive'].map(type=>{const policy=policies.find(p=>p.vehicleId===row.id&&p.type===type);return policy?`${policyType(type)}: ${prettyDate(policy.endDate)}`:`${policyType(type)}: לא הוזן תוקף הביטוח`;})),(()=>{const test=reminders.filter(r=>r.vehicleId===row.id&&r.type==='Test').sort((a,b)=>String(b.expiryDate).localeCompare(String(a.expiryDate)))[0];return test?`תוקף טסט: ${prettyDate(test.expiryDate)}`:'לא הוזן תוקף טסט';})()].filter(Boolean).join(' · ')})}/>
