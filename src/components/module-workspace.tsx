...
type Field = { key: string; label: string; type?: 'text' | 'number' | 'date' | 'license'; min?: number; max?: number; step?: string; optional?: boolean; hidden?: boolean; compact?:boolean; options?: { value: string; label: string }[] };
...
function LicensePlateInput({value,year,month,onChange}:{value:string;year:number;month:number;onChange:(value:string)=>void}){
  ...
  function update(position:number,digit:string){const chars=[...formatted];chars[position]=digit.replace(/\D/g,'').slice(-1)||'x';onChange(chars.join(''));if(chars[position]!=='x'){const next=slots[slots.indexOf(position)+1];if(next!==undefined)(document.querySelector(`[data-plate-slot="${next}"]`) as HTMLInputElement|null)?.focus();}}
  return <div className="license-plate-input" dir="ltr" aria-label="מספר לוחית רישוי רכב">{[...pattern].map((char,index)=>char==='-'?<span key={index}>-</span>:<input key={index} data-plate-slot={index} inputMode="numeric" pattern="[0-9]" maxLength={1} placeholder="x" value={/\d/.test(formatted[index]??'')?formatted[index]:''} onChange={event=>update(index,event.target.value)} onKeyDown={event=>{if(event.key==='Backspace'&&!event.currentTarget.value){const previous=slots[slots.indexOf(index)-1];if(previous!==undefined){update(previous,'');(document.querySelector(`[data-plate-slot="${previous}"]`) as HTMLInputElement|null)?.focus();}}}}/> )}</div>;
}
...
function RecordPanel({ kind, title, action, fields, records, defaults, describe, disabled = false, externalDraft, onDraftConsumed, confirmDeletion = true, hideAction = false, openSignal = 0 }: {
  ...
  defaults: Record<string, string | number | null>; describe: (record: ModuleRecord) => { title: string; detail: string; status?:string };
  disabled?: boolean;
  ...
  const closeOnBackdrop=(event:React.MouseEvent<HTMLDialogElement>)=>{if(event.target===event.currentTarget&&!busy)event.currentTarget.close();};
  return <section className={`journal-panel ${kind==='water'?'water-journal':''} ${kind==='vehicles'?'vehicles-panel':''}`}>
    ...
    {visibleRecords.length === 0 ? <div className="journal-empty">עדיין אין רשומות. {disabled ? 'היסטוריית התדלוקים תופיע כאן.' : `אפשר לבחור „${action}” כדי להתחיל.`}</div> :
      <div className="journal-list">{visibleRecords.map(record => {
        ...
        return <article key={record.id} className="journal-row"><div><h3>{text.title}</h3><p>{text.detail}</p></div>{text.status&&<span className={`record-status ${text.status==='פתוח'?'open':''}`}>{text.status}</span>}<div className="row-actions">
    ...
    <dialog className={`modal record-modal ${kind==='fuel'?'fuel-entry-modal':''}`} ref={dialog} onClick={closeOnBackdrop} onCancel={event => { if (busy) event.preventDefault(); }}><form onSubmit={submit}>
      ...
      <div className="record-form-grid">{fields.filter(field=>!field.hidden).map(field => <label className={field.compact?'compact-field':''} key={field.key} htmlFor={`${kind}-${field.key}`}>{field.label}{field.optional && <span className="optional">(לא חובה)</span>}
...
function CarNotifications({summary}:{summary:Summary}){
  ...
  return <section className="car-notifications"><div><ShieldCheck/><span><strong>התראות תחזוקת רכב</strong><small>ביטוחים וטסטים יישלחו גם כשהאתר סגור</small></span></div><label className="water-reminder-toggle"><span>{enabled?'פעילות':'כבויות'}</span><input type="checkbox" checked={enabled} disabled={busy} onChange={event=>void toggle(event.target.checked)}/><i/></label>{enabled&&!deviceReady&&<button className="button secondary" disabled={busy} onClick={()=>void enableDevice()}>הפעלת התראות במכשיר הזה</button>}{message&&<p className="feedback">{message}</p>}{error&&<p className="error-text">{error}</p>}</section>;
}
...
export function ModuleWorkspace({ module, summary, water, vehicles, fuel, nutrition, policies, reminders, expenses, fuelPrice }: { module: 'water' | 'car' | 'nutrition'; summary: Summary; water: ModuleRecord[]; vehicles: ModuleRecord[]; fuel: ModuleRecord[]; nutrition: ModuleRecord[]; policies: ModuleRecord[]; reminders: ModuleRecord[]; expenses:ModuleRecord[]; fuelPrice: FuelPrice | null }) {
  ...
  return <div className={`module-workspace ${module}-workspace`}>
    ...
    <header className={`page-heading ${module==='water'?'water-page-heading':''}`}><div>{module==='nutrition'&&<span className="eyebrow">היום־יום שלך</span>}<h1>{title}</h1>{module==='nutrition'&&<p>הארוחות והיעדים היומיים מול העיניים.</p>}</div><span className={`module-icon ${module}-card`}><Icon/></span></header>
    <section className="module-stats" aria-label={`סיכום ${title}`}>
      ...
      {module === 'car' && <><FuelPriceCard data={fuelPrice} format={formatMoney}/><div><small>עלות הדלק החודש</small><strong>{formatMoney(summary.fuelCost)}</strong></div><div className="car-vehicle-stat"><small>פרטי הרכב</small><strong className="vehicle-summary">{vehicles[0]?.name||'טרם הוגדר רכב'}</strong><small>{vehicles[0]?[vehicles[0].year&&vehicles[0].roadMonth?`${String(vehicles[0].roadMonth).padStart(2,'0')}/${vehicles[0].year}`:null,vehicles[0].licensePlate].filter(Boolean).join(' · '):'הוספת רכב תציג כאן את פרטיו'}</small></div></>}
    ...
    {module === 'car' && <>
      <RecordPanel kind="vehicles" title="רכבים" action="הוספת רכב" records={vehicles} fields={[{key:'name',label:'סוג הרכב',max:100},{...numberField('year','שנת עלייה לכביש',2100,1900),compact:true},{key:'roadMonth',label:'חודש עלייה לכביש',options:Array.from({length:12},(_,index)=>({value:String(index+1),label:String(index+1).padStart(2,'0')})),compact:true},{key:'licensePlate',label:'מספר לוחית רישוי רכב',type:'license'},{...numberField('odometerKm','קילומטראז׳ נוכחי',10000000),compact:true},{...numberField('fuelTankLiters','נפח מכל הדלק (ליטר)',1000,1,'0.1'),compact:true}]} defaults={{name:'',licensePlate:'',year:new Date().getFullYear(),roadMonth:new Date().getMonth()+1,odometerKm:0,fuelTankLiters:''}} describe={row=>({title:String(row.name),detail:[row.year&&row.roadMonth?`עלה לכביש ${String(row.roadMonth).padStart(2,'0')}/${row.year}`:row.year,row.licensePlate,`${Number(row.odometerKm).toLocaleString('he-IL')} ק״מ`,row.fuelTankLiters?`מכל ${row.fuelTankLiters} ליטר`:null].filter(Boolean).join(' · ')})}/>
      <CarNotifications summary={summary}/>
