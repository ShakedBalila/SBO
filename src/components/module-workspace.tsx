...
const formatOne = (value: unknown) => Number(Number(value||0).toFixed(1)).toLocaleString('he-IL',{maximumFractionDigits:1});
...
function fuelEfficiency(row:ModuleRecord){const liters=Number(row.liters),distance=Number(row.actualDistanceKm);return liters>0&&distance>0?distance/liters:null;}
function MonthlyFuelEfficiency({fuel}:{fuel:ModuleRecord[]}){
  const now=new Date(),months=Array.from({length:6},(_,i)=>{const date=new Date(Date.UTC(now.getUTCFullYear(),now.getUTCMonth()-5+i,1));return {key:date.toISOString().slice(0,7),label:new Intl.DateTimeFormat('he-IL',{month:'short',timeZone:'UTC'}).format(date),distance:0,liters:0};});
  for(const row of fuel){if(!fuelEfficiency(row))continue;const month=months.find(value=>value.key===String(row.date).slice(0,7));if(month){month.distance+=Number(row.actualDistanceKm);month.liters+=Number(row.liters);}}
  const values=months.map(month=>({...month,value:month.liters?month.distance/month.liters:0})),max=Math.max(1,...values.map(month=>month.value));
  return <section className="cost-chart efficiency-chart"><div className="section-heading"><div><h2>יחס קילומטר לליטר לפי חודשים</h2><p>ממוצע חודשי משוקלל לפי המרחק בפועל וכמות הדלק</p></div></div><div className="bars">{values.map(month=><div className="bar-column" key={month.key}><small>{month.value?`${month.value.toFixed(1)} ק״מ/ל׳`:'—'}</small><div className="bar efficiency" style={{height:`${Math.max(4,month.value/max*180)}px`}}/><strong>{month.label}</strong></div>)}</div></section>;
}
...
export function ModuleWorkspace({ module, summary, water, vehicles, fuel, nutrition, policies, reminders, expenses, fuelPrice }: { module: 'water' | 'car' | 'nutrition'; summary: Summary; water: ModuleRecord[]; vehicles: ModuleRecord[]; fuel: ModuleRecord[]; nutrition: ModuleRecord[]; policies: ModuleRecord[]; reminders: ModuleRecord[]; expenses:ModuleRecord[]; fuelPrice: FuelPrice | null }) {
  ...
  const policyType = (value: unknown) => ({ Mandatory: 'חובה', Comprehensive: 'מקיף + צד ג׳', 'Third party': 'צד ג׳' }[String(value)] ?? String(value));
  ...
  const insuranceIncomplete=vehicles.some(vehicle=>{const types=new Set(policies.filter(policy=>policy.vehicleId===vehicle.id).map(policy=>String(policy.type)));return !types.has('Mandatory')||(!types.has('Third party')&&!types.has('Comprehensive'));});
  const currentFuel=fuel.filter(row=>String(row.date).slice(0,7)===summary.today.slice(0,7)&&fuelEfficiency(row));
  const monthlyEfficiency=currentFuel.length?currentFuel.reduce((sum,row)=>sum+Number(row.actualDistanceKm),0)/currentFuel.reduce((sum,row)=>sum+Number(row.liters),0):null;
  ...
  return <div className={`module-workspace ${module}-workspace`}>
    ...
    <section className="module-stats" aria-label={`סיכום ${title}`}>
      ...
      {module === 'car' && <><FuelPriceCard data={fuelPrice} format={formatMoney}/><div><small>עלות הדלק החודש</small><strong>{formatMoney(summary.fuelCost)}</strong></div><div><small>ממוצע קילומטר לליטר החודש</small><strong>{monthlyEfficiency?monthlyEfficiency.toFixed(1):'—'} <span>{monthlyEfficiency?'ק״מ/ל׳':''}</span></strong></div><div className="car-vehicle-stat"><VehicleBrandLogo name={String(vehicles[0]?.name??'')}/><small>פרטי הרכב</small><strong className="vehicle-summary">{vehicles[0]?.name||'טרם הוגדר רכב'}</strong><small>{vehicles[0]?[vehicles[0].year&&vehicles[0].roadMonth?`${String(vehicles[0].roadMonth).padStart(2,'0')}/${vehicles[0].year}`:null,vehicles[0].licensePlate].filter(Boolean).join(' · '):'הוספת רכב תציג כאן את פרטיו'}</small></div><CarNotifications summary={summary}/></>}
    ...
    {module === 'water' && <>
      ...
      <div className="quick-water"><h2>הוספת שתייה מהירה</h2><div>{[200, 250, 500, 750, 1000].map(amount => <button key={amount} className="button secondary water-quick-button" disabled={busy || !ready} onClick={async () => {
