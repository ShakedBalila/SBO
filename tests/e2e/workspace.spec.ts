...
test('registration, CRUD, isolation, responsive navigation and logout', async ({ page, browser, request }) => {
  ...
  await page.getByRole('link', { name: 'משימות', exact: true }).first().click();
  await page.getByRole('button', { name: 'משימה חדשה', exact: true }).click();
  await page.getByLabel('שם המשימה').fill('Plan the week');
  await page.getByLabel('תיאור', { exact: false }).fill('Choose three priorities for the week.');
  await page.getByLabel('עדיפות', { exact: true }).selectOption('HIGH');
  await page.getByLabel('תאריך יעד', { exact: false }).fill('2026-09-15');
  ...
  const id = tasks[0].id;
  expect(tasks[0].dueDate).toBe('2026-09-15');
  ...
  const taskData = { title: 'Attempt to change another account', description: '', status: 'TODO', priority: 'LOW', dueDate: null };
  ...
  expect((await page.request.post('/api/tasks', { headers: origin, data: { ...taskData, dueDate: '2026-02-30' } })).status()).toBe(400);
  await other.close();
  await page.getByRole('button', { name: 'עריכת Plan the week', exact: true }).click();
  await page.getByLabel('שם המשימה').fill('Plan a focused week');
  await page.getByLabel('מצב', { exact: true }).selectOption('IN_PROGRESS');
  ...
  await page.getByRole('button', { name: 'שמירת המשימה', exact: true }).click();
  await expect(page.getByRole('heading', { name: 'Plan a focused week' })).toBeVisible();
  await page.getByRole('button', { name: 'מחיקת Plan a focused week', exact: true }).click();
  await page.getByRole('button', { name: 'מחיקת משימה', exact: true }).click();
  await expect(page.getByRole('heading', { name: 'Plan a focused week' })).not.toBeVisible();
