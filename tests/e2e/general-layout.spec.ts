...
test('fluid shell, water history, vehicle logo and food bank',async({page})=>{
  ...
  try{
    ...
    expect((await page.request.post('/api/modules/water',{headers,data:{amountMl:325,date:'2026-09-15'}})).ok()).toBeTruthy();
    ...
    for(const width of [320,430,507,600,834,1024,1440,1920]){
      ...
      for(const route of ['/','/tasks','/water','/car','/nutrition']){await page.goto(route);await page.locator('main:visible').waitFor();expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBeTruthy();if([430,507,834,1440].includes(width)){const name=route==='/'?'home':route.slice(1);await page.screenshot({path:`test-results/general-${name}-${width}.png`,fullPage:true});}}
      await page.goto('/');await page.screenshot({path:`test-results/general-home-${width}.png`,fullPage:true});
      if(width<=900){await page.evaluate(()=>window.scrollTo(0,document.body.scrollHeight));const nav=await page.locator('.bottom-nav').boundingBox();expect(nav).not.toBeNull();expect(Math.abs(nav!.y+nav!.height-height)).toBeLessThan(2);}
    }
    await page.setViewportSize({width:430,height:932});await page.goto('/water');
    await expect(page.locator('.water-history-days button')).toHaveCount(7);await expect(page.locator('.water-history-days button[aria-pressed="true"]')).toHaveText('ג');await expect(page.getByRole('heading',{name:'325 מ״ל'})).toBeVisible();
